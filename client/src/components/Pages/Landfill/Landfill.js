...
import LandfillTable from './LandfillTable/LandfillTable';
...
const Landfill = () => {
    ...
    // Extract latitude and longitude from landfills data
    const coordinates = landfillData.map(landfill => {
        const [latitude, longitude] = landfill.coordinates.split(',').map(coord => parseFloat(coord.trim()));
